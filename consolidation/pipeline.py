...
def consolidate_types_func(mentions, annset, idx_dict, types):
    for mention in mentions:
        ann = annset[idx_dict[mention['id']]]
        ann['type'] = type
        ...
        for cluster in clusters:
            ...
            for mention in cluster['mentions']:
                ...
                if 'types' in ann['features']:
                    types.append(' '.join(ann['features']['types']))
                else:
                    types.append(ann['type'])
            most_common_type = max(set(types), key=types.count)
            types = most_common_type.split(' ')
...
doc = res.json()
# res = requests.post('http://10.0.0.113:10880/api/blink/biencoder/mention/doc', json = doc)
# doc = res.json()
# res = requests.post('http://10.0.0.113:10880/api/nilprediction/doc', json = doc)
# doc = res.json()
# res = requests.post('http://10.0.0.113:10880/api/clustering', json = doc)
# doc = res.json()
