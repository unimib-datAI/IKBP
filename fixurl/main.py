...
def queue_doc_in_callback(ch, method, properties, body):
    ...
    try:
        for anno in doc['annotation_sets']['entities_merged']['annotations']:
            if anno['features']['linking']['top_candidate']['indexer'] == 119:
                myid = anno['features']['linking']['top_candidate']['wikipedia_id']
                myurl = f'https://vm.chronos.disco.unimib.it:17474/#{myid}'
                #print(myurl)
                anno['features']['linking']['top_candidate']['url'] = myurl
                anno['features']['url'] = myurl
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--queue", type=str, default="fixurl", help="rabbitmq queue root",
    )
