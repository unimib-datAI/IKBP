        ...
        for (const annset of document.annotation_sets) {
            ...
            if (annset.name.startsWith("entities")) {
                for (const annot of annset.annotations) {
                    ...
                    if (
                        !("mention" in annot.features) &&
                        typeof document.text === "string"
                    ) {
                        // Guard against invalid start/end types as well
                        const s =
                            typeof annot.start === "number" ? annot.start : 0;
                        const e = typeof annot.end === "number" ? annot.end : s;
                        annot.features.mention = document.text.substring(s, e);
                    }
            ...
            if (annset.name.startsWith("entities_consolidated")) {
                // Only operate on preview if it's a string (avoid undefined errors)
            ...
            if (typeof document.preview === "string") {
                document.preview = document.preview.replace(
                    regexPattern,
